...
@section('content')
    <div class="box">
        <div class="overlay">
            <h2>البحث بحصر الادارة العامة للتحقيقات(جُنح)  </h2>
            <!--start form -->
            <div class="search">
                <form action="{{ route('child') }}" method="POST" role="search">
                    {{ csrf_field() }}
                    <div class="input-group">
                        <input class="form-control number-search" type="number" name="p" min="1" lang="en">
                        <label class="slash">/</label>
                        <select style="background: rgba(255, 255, 255, .4);"  class="form-control select2" name="q">
                            <?php
                            for($i = 2014; $i <= 2040 ; $i++) {  ?>
