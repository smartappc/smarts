...
@section('content')
    ...
    <div class="box">
        <div class="overlay">
            <h2>البحث باسم الضابط  </h2>

            <!--start form -->
            <div class="search officers">
                <form action="{{ route('officer') }}" method="POST" role="search">
                    {{ csrf_field() }}
                    <div class="input-group">
                        <select  class="form-control" name="q" >
                            <option value="">.....</option>
                            @foreach(\App\officer::all() as $f)
                                <option value="{{$f->id}}" >{{ $f->name }}</option>
                            @endforeach
                        </select>

                        <button type="submit" class="btn btn-default officer">
